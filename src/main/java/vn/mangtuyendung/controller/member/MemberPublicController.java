...
/*
 * MemberPublicController.java
 * 
 * Version 1.0
 *
 * Date 01/01/2013
 * 
 * Member public controller
 * 
 * Modification Logs:
...
@RequestMapping("/hoso/{username}")
@Controller
public class MemberPublicController extends SiteAbstractController {

    private final Logger logger = LoggerFactory.getLogger(MemberPublicController.class);
    @Autowired
    ...
    public String viewMember(@PathVariable("username") String username, Model model) {
        try {
            this.logger.debug("View the profile " + username);
            ...
            this.logger.error("View member " + username + " error", ex);
        }
